...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        '''
        根据论文 'Feature pyramid networks for object detection' 中的描述，
        resnet输出的特征图共有5个尺度，分别为C1，C2，C3，C4，C5
        为FPN做准备，我们需要记录resnet的中间特征图C3，C4，C5
        '''
        x = self.layer1(x)
        C2 = x
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        # 故，当新定义的网络（model_dict）和预训练网络（pretrained_dict）
        # 的层名不严格相等时，需要先将pretrained_dict里不属于model_dict的键剔除掉
        '''
